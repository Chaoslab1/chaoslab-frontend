...
This guide explains how to integrate Chaoslab webhooks with Zapier for no-code automation.

## Overview

Chaoslab provides flat, robust webhook payloads optimized for no-code platforms like Zapier and Make.com. The webhook structure is designed to be easily parsed and mapped without complex nested objects.
...
### 2. Configure Chaoslab

Add the following environment variables to your Chaoslab application:
...
```bash
curl -X POST https://your-chaoslab-domain.com/api/webhooks/test \
...
Chaoslab signs all webhook payloads using HMAC-SHA256. The signature is sent in the `X-Webhook-Signature` header.
...
**Trigger**: Chaoslab Webhook (transaction.completed)
...
**Trigger**: Chaoslab Webhook (transaction.failed)
...
3. Check webhook delivery logs in Chaoslab
...
1. Check the Chaoslab application logs
