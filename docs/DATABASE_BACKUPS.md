...
Database: Chaoslab_stellar
Backup Bucket: chaoslab-backups

Starting backup to /tmp/backups/2026-04-27T15-32-00Z.dump...
✓ Backup dump created: 245.67 MB
Encrypting backup...
Uploading to S3...
✓ Backup uploaded to S3: s3://chaoslab-backups/backups/2026-04-27T15-32-00Z.dump.enc

✅ Backup Successful!
   Backup ID: 2026-04-27T15-32-00Z
   S3 URL: s3://chaoslab-backups/backups/2026-04-27T15-32-00Z.dump.enc
   Size: 245.67 MB
   Duration: 87234ms
...
spec:
  ...
  jobTemplate:
    spec:
      template:
        spec:
          serviceAccountName: chaoslab
          containers:
            - name: backup
              image: chaoslab:latest
              command: ["npm", "run", "backup:create"]
              env:
                - name: DATABASE_URL
                  valueFrom:
                    secretKeyRef:
                      name: chaoslab-secrets
                      key: database-url
                - name: BACKUP_BUCKET
                  value: chaoslab-backups
                - name: AWS_REGION
                  value: us-east-1
...
# Optional
BACKUP_BUCKET=chaoslab-backups              # Default
...
resource "aws_cloudwatch_metric_alarm" "backup_bucket_size" {
  alarm_name          = "chaoslab-backup-bucket-size"
...
```
s3://chaoslab-backups-logs/backups/
```
...
```bash
aws s3 ls s3://chaoslab-backups/backups/ \
  --recursive \
  --human-readable \
  --summarize
```
...
aws s3 cp s3://chaoslab-backups/backups/2026-04-27T15-32-00Z.dump.enc .
...
aws s3api head-bucket --bucket chaoslab-backups
