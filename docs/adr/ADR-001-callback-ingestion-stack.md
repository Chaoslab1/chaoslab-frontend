...
The chaoslab platform receives payment callbacks from Chaoslab providers (MTN, Airtel, Orange) at high volume. The current ingestion path is a Node.js Express handler that validates the payload and writes to a PostgreSQL queue table. Under load testing this path shows:
