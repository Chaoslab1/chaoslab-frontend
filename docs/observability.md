...
```logql
sum(rate({container="Chaoslab_app"} | json | level="ERROR" [5m]))
/
sum(rate({container="Chaoslab_app"} [5m]))
* 100
```
...
```logql
quantile_over_time(0.99, {container="Chaoslab_app"} | json | unwrap duration [5m])
```
...
```logql
{container="Chaoslab_app"} | json | level="SECURITY"
```
...
```logql
{container="Chaoslab_app"} | json | level="AUDIT"
```
