...
This integrates PagerDuty Events API V2 with the chaoslab platform to automatically alert on-call engineers when provider error rates exceed 15% in a 5-minute sliding window.
...
PAGERDUTY_DEDUP_KEY=chaoslab      # Dedup prefix (optional, defaults to chaoslab)
...
```json
{
  ...
  "dedup_key": "chaoslab-stripe-error-rate",
  "payload": {
