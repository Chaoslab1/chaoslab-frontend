...
4. Configure bucket:
   - **Bucket name**: `chaoslab-kyc-documents` (must be globally unique)
...
```bash
# Create bucket
aws s3 mb s3://chaoslab-kyc-documents --region us-east-1

# Enable versioning
aws s3api put-bucket-versioning \
  --bucket chaoslab-kyc-documents \
...
3. Configure user:
   - **User name**: `chaoslab-s3-uploader`
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:s3:::chaoslab-kyc-documents",
        "arn:aws:s3:::chaoslab-kyc-documents/*"
      ]
    }
  ]
...
4. Click **Next: Review**
5. **Policy name**: `ChaoslabKYCUploadPolicy`
...
3. Search for `ChaoslabKYCUploadPolicy`
4. Check the policy
...
```bash
# Create IAM user
aws iam create-user --user-name chaoslab-s3-uploader
...
aws iam create-policy \
  --policy-name ChaoslabKYCUploadPolicy \
  --policy-document file://kyc-upload-policy.json

# Attach policy to user (replace ACCOUNT_ID with your AWS account ID)
aws iam attach-user-policy \
  --user-name chaoslab-s3-uploader \
  --policy-arn arn:aws:iam::ACCOUNT_ID:policy/ChaoslabKYCUploadPolicy

# Create access key
aws iam create-access-key --user-name chaoslab-s3-uploader
```
...
AWS_S3_BUCKET=chaoslab-kyc-documents
```
...
aws s3 mb s3://chaoslab-kyc-logs
...
aws s3api put-bucket-logging \
  ...
  --bucket-logging-status '{
    "LoggingEnabled": {
      "TargetBucket": "chaoslab-kyc-logs",
