...
| ------------------------------------------ | ------------------------ | ------------------------------------------- |
| `FRONTEND_URL`                             | URL for invitation links | `https://app.Chaoslab.com`               |
| `SENDGRID_MERCHANT_INVITATION_TEMPLATE_ID` | SendGrid template ID     | -                                           |
| `EMAIL_FROM`                               | Sender email address     | `"Chaoslab" <no-reply@Chaoslab.com>` |
...
```bash
psql -d CHAOSLAB -f migrations/20260601_create_merchants_table.sql
```
