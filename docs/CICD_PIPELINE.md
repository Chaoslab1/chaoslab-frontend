...
This document provides comprehensive documentation for the GitHub Actions CI/CD pipeline that automates testing, building, and deployment of the Chaoslab to Stellar backend service. The pipeline ensures code quality, generates deployable artifacts, and automates staging deployments.
...
- `{repository}:{commit-sha}` - Always applied (e.g., `chaoslab/chaoslab-docs:a1b2c3d`)
- `{repository}:{branch-name}` - Always applied (e.g., `chaoslab/chaoslab-docs:main`)
...
| `MTN_API_KEY`           | MTN Chaoslab API key                 | `xxx-xxx-xxx`                         |
| `MTN_API_SECRET`        | MTN Chaoslab API secret              | `xxx`                                 |
...
- Chaoslab provider credentials (MTN, Airtel, Orange)
...
```markdown
![CI](https://github.com/chaoslab/chaoslab-docs/actions/workflows/ci.yml/badge.svg)
```
...
```markdown
![Coverage](https://codecov.io/gh/chaoslab/chaoslab-docs/branch/main/graph/badge.svg)
```
