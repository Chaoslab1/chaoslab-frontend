...
This implementation provides SEP-10 authentication for the chaoslab platform, allowing users to authenticate using their Stellar accounts. SEP-10 is a Stellar Ecosystem Proposal that defines a standard way to authenticate users using their Stellar account addresses.
...
```bash
curl "https://api.Chaoslab.com/sep10/auth?account=GABC123..."
```
...
STELLAR_HOME_DOMAIN=api.Chaoslab.com
...
const challengeResponse = await fetch(
  "https://api.Chaoslab.com/sep10/auth?account=GABC123...",
);
...
const verifyResponse = await fetch("https://api.Chaoslab.com/sep10/auth", {
  method: "POST",
...
const apiResponse = await fetch(
  "https://api.Chaoslab.com/api/transactions",
  {
